...
public class XiVersion {
    ...
    public static void main(String[] args) {
        if (args.length >0) {
            JTextArea jtx = new JTextArea("Current Version " + getVersionString() + "\n"+changes);
            JScrollPane jsp = new JScrollPane(jtx);
            jsp.setPreferredSize(new Dimension(400,200));
            if (args[0].toLowerCase().trim().contentEquals("--gui")) {
                javax.swing.JOptionPane.showMessageDialog(null, jsp, "Version " + getVersionString(), javax.swing.JOptionPane.PLAIN_MESSAGE);
                return;
        ...
        System.out.println("Current Version: "+ getVersionString());
        StringBuilder sb = new StringBuilder();
        String[] versions = changes.split("(\n|^)Version");
